...
'''
#获得机器的分布
def machine_distribution(machine_model,data,vocab,forward_passes):
    labels = list()
    dropout_predictions = np.empty((0, 969, 2))
    for i in range(forward_passes):
        model=machine_model
        for idx, (text, label) in enumerate(tqdm(data)):
        ...
        dropout_predictions = np.vstack((dropout_predictions,
                                         predictions[np.newaxis, :, :]))   # dropout_predictions -> shape -> (forward_passes, n_samples, n_classes)

    # 计算均值
    mean = np.mean(dropout_predictions, axis=0) # mean -> shape -> (n_samples, n_classes)
    machine_mean = torch.from_numpy(mean).cuda()
    # print("mean:", mean)
    # 计算方差
    machine_variance = np.var(dropout_predictions, axis=0) # variance -> shape -> (n_samples, n_classes)
    # print("variance:", variance)
    return machine_mean,machine_variance
'''


# def machine_distribution(machine_model,train_x,batch_size,forward_passes):
#     dropout_predictions = np.empty((0,batch_size,2))
#     for i in range(forward_passes):
#         pred = machine_model(train_x) #tensor(128,2)
#         pred=pred.detach().cpu().numpy()
#         dropout_predictions = np.vstack((dropout_predictions, pred[np.newaxis, :, :]))   # dropout_predictions -> shape -> (forward_passes, batch_size, n_classes)
#     # 计算均值
#     mean = np.mean(dropout_predictions, axis=0) # mean -> shape -> (n_samples, n_classes)
...
#     # print("mean:", mean)
#     # 计算方差
#     variance = np.var(dropout_predictions, axis=0) # variance -> shape -> (n_samples, n_classes)
...
class WordCNNForClassification(nn.Module):
    ...
    def __init__(
        ...
        vocab = pickle.load(f2)
        self.emb_layer = nn.Embedding(len(vocab), embed_size)  # embedding层
        embeding_vector = np.load('../../../correct_score_(true_rate_to_num_for_diff)/glove_50d_wordlist.npy',
                                  allow_pickle=True)
        self.emb_layer.weight.data.copy_(torch.from_numpy(embeding_vector))
        self.emb_layer.weight.requires_grad = True
...
class ELL(nn.Module):
    ...
    def forward(self, samples, y_true, machine_mu, machine_var, user_theta, encoder_mu,
        ...
        for i in range(batch_size):  # 一个样本一个样本进行损失计算
            ...
            each_samples2 = self.sigmoid(each_samples)

            beta_dist = td.beta.Beta(beta_a, beta_b)
            beta_log_likelihood = beta_dist.log_prob(each_samples2)  # 对数似然
...
# 模型训练
def train(encoder_model,  user_model, train_data, ver_data, vocab, forward_passes, epoch, batch_size,
    ...
    # 定义损失函数和优化器
    criterion = ELL()
    encoder_optimizer = torch.optim.SGD(encoder_model.parameters(), lr=4e-2)
    # encoder_optimizer = torch.optim.Adagrad(encoder_model.parameters(), lr=4e-2, lr_decay=0.0001, weight_decay=0.0001,
    ...
    for epoch in tqdm(range(epoch)):
        ...
        # 开始验证
        print("开始验证++++++++++++++++++++++++++")
        evaluations = test( user_model, encoder_model, ver_data, test_batch_size_value, forward_passes,
                           vocab, result_save_path)
        ...
        if ver_inte_acc > best_inte_acc:
            ...
            torch.save(encoder_model.state_dict(),
                       result_save_path + 'CNN_dropout_score_weigth_encoder_model_parameter_bata230420.pkl')
...
# 不打开dropout测试
def test(user_model, encoder_model, test_data, batch_size, forward_passes, vocab, result_save_path):
    print('test model')
    encoder_model = encoder_model.cuda()
    user_model = user_model.cuda()
    encoder_model.eval()
    ...
    mean_all_gap = np.empty((0, 1))
    std_all_gap = np.empty((0, 1))
    machine_mean_all = np.empty((0, 1))
    machine_std_all= np.empty((0, 1))
    user_mean_all = np.empty((0, 1))
    user_std_all= np.empty((0, 1))
    encoder_mean_all = np.empty((0, 1))
    encoder_std_all= np.empty((0, 1))
    for idx, (text, label, user,machine_mean,machine_variance) in enumerate(tqdm(test_data)):
        ...
        machine_variance2 = machine_variance.reshape(batch_size, 1)
        encoder_std2 = encoder_std.reshape(batch_size, 1)
        user_mean2 = user_mean.reshape(batch_size, 1)
        user_variance2 = user_variance.reshape(batch_size, 1)
        machine_mean_all = np.vstack((machine_mean_all, machine_mean2.cpu().detach().numpy()))
        machine_std_all = np.vstack((machine_std_all, machine_variance2.cpu().detach().numpy()))
        user_mean_all = np.vstack((user_mean_all, user_mean2.cpu().detach().numpy()))
        user_std_all = np.vstack((user_std_all, user_variance2.cpu().detach().numpy()))
        encoder_mean_all = np.vstack((encoder_mean_all, encoder_mu2.cpu().detach().numpy()))
        encoder_std_all = np.vstack((encoder_std_all, encoder_std2.cpu().detach().numpy()))

        results = np.concatenate((machine_mean_all, machine_std_all, user_mean_all, user_std_all,encoder_mean_all,encoder_std_all),axis=1)
        results2 = pd.DataFrame(results)
        results2.to_csv(result_save_path+'all_distribution.csv',index=0)

    evaluations = [machine_avg_acc, user_avg_acc, avg_acc, avg_pre, avg_rec, avg_f, avg_auc]
    return evaluations
    '''
        # 计算encoder和machine的分布差距
        machine_mean2 = machine_mean.reshape(batch_size, 1)
    ...
    temp_list = []
    data_size=len(test_data)*batch_size
    for i in range(1, data_size + 1):
        temp_list.append(i)

    # for b in range(3):
    #     plt.plot(temp_list, mean_epoch_gap[b], label=b)
    mean_all_gap2=np.sort(mean_all_gap,axis=0)
    for i in range(len(temp_list)):
        plt.bar(temp_list[i], mean_all_gap2[i])
    plt.title("CNN machine_mean VS output_mean")
    # 设置 x,y 轴取值范围
    plt.xlabel("samples")
    plt.ylabel("△")
    # plt.legend()
    ...
    std_all_gap2=np.sort(std_all_gap,axis=0)
    for i in range(len(temp_list)):
        plt.bar(temp_list[i], std_all_gap2[i])
    plt.title("CNN machine_std VS output_std")
    # 设置 x,y 轴取值范围
    plt.xlabel("samples")
    plt.ylabel("△")
    # plt.legend()
    ...
    print('machine_acc:', machine_acc, "machine_pre:", machine_pre, "machine_rec:", machine_rec,
          ...
          "user_f:", user_f, "user_auc_result:", user_auc_result,'acc:', acc, "pre:", pre, "rec:", rec,
         "f:", f, "auc_result:", auc_result)
    # print('machine_acc:', machine_acc,'user_acc:', user_acc,'acc:', acc, "pre:", pre, "rec:", rec,
    #      "f:", f, "auc_result:", auc_result)

    print("test_encoder_mu", encoder_mu)
    print("test_encoder_mu_label", encoder_mu_label)
...
def main():
    ...
    filename = 'cnn_news_user_weight_beta_8_1_1_230712.py'  # 当前路径
    ...
    # train_path = open('../dataset/mini_val_data_with_cnn_machine_distribution.pkl', 'rb')
    # train_file = pickle.load(train_path)

    test_path = open('../dataset/mini_test_data_with_cnn_machine_distribution.pkl', 'rb')
    ...
    encoder_result_save_path = r'../results/cnn_news_user_weight_beta/7_2_1/230712/gausi_beta_pre_loss/round2_230713/'
    ...
    train_batch_size_value = 16
    test_batch_size_value = 64
    # 构建DataLoder
    ...
    #       ver_data=test_loader, vocab=vocab, forward_passes=forward_passes, epoch=60, batch_size=train_batch_size_value,
    ...
    encoder_model.load_state_dict(
        torch.load(encoder_result_save_path + 'CNN_dropout_score_weigth_encoder_model_parameter_bata230420.pkl',
                   map_location=torch.device('cpu')))
    print("开始测试————————————————————————————————")
    # 测试结果
    ...
    test(user_model=user_model, encoder_model=encoder_model, test_data=test_loader,
         batch_size=test_batch_size_value, forward_passes=forward_passes, vocab=vocab,
         result_save_path=encoder_result_save_path)
