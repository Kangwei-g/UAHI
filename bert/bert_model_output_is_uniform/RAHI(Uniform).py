...
def reparameterize(mean, logstd, batch_size): #mean={Tensor:batch_size}  logstd={Tensor:batch_size}
    samples = torch.empty(batch_size, 0)
    std = torch.exp(logstd)  # 得到标准差
    std_size = std.size()
    # 根据均值和方差计算 a 和 b
    # 均值 μ = (a + b) / 2, 方差 σ² = (1/12) * (b - a)²
    # 从中解出 a 和 b
    a = mean - np.sqrt(3) * std  #a={Tensor:batch_size} tensor([-0.5561, -0.6360, -1.0198, -0.5858, -0.8977, -0.9859, -1.1089, -0.5491,-0.6532, -0.6156, -0.5376, -0.6603, -0.5875, -0.5406, -0.9302, -0.5617], device='cuda:0', grad_fn=<SubBackward0>)
    b = mean + np.sqrt(3) * std  #b={Tensor:batch_size}  tensor([1.4723, 1.5478, 1.9507, 1.4722, 1.8133, 1.9127, 2.0307, 1.4651, 1.5500,1.5313, 1.4648, 1.5894, 1.4858, 1.4578, 1.7983, 1.4845],device='cuda:0', grad_fn=<AddBackward0>)

    for i in range(20):
        random_value1 = np.random.rand(batch_size)
        random_value = torch.tensor(random_value1).cuda()
        # eps = torch.randn_like(std) #从标准正态分布中采样一个ε（epsilon），大小与标准差一样。
        sample = a+(b-a)*random_value
        #sample = np.random.uniform(a,b).detach().numpy()  # 参数分别为均值、标准差和生成样本数
...
def test(user_model, encoder_model, test_data, batch_size, forward_passes, vocab, result_save_path):
    ...
    mean_all_gap = np.empty((0, 1))
    std_all_gap = np.empty((0, 1))
    user_mean_all_gap = np.empty((0, 1))
    user_std_all_gap = np.empty((0, 1))
    ...
    for idx, (text, label, user,machine_mean,machine_variance) in enumerate(tqdm(test_data)):
        ...
        user_mean_gap = abs(user_mean2 - encoder_mu2)
        user_std_gap = abs(user_variance2 - encoder_std2)
        user_mean_all_gap = np.vstack((user_mean_all_gap, user_mean_gap.cpu().detach().numpy()))
        user_std_all_gap = np.vstack((user_std_all_gap, user_std_gap.cpu().detach().numpy()))
    ...
    plt.title("LSTM machine_mean VS output_mean")
    # 设置 x,y 轴取值范围
    plt.xlabel("samples")
    plt.ylabel("△")
    plt.show()
    ...
    plt.title("LSTM machine_std VS output_std")
    # 设置 x,y 轴取值范围
    plt.xlabel("samples")
    plt.ylabel("△")
    plt.show()
    ...
    user_mean_all_gap2 = np.sort(user_mean_all_gap, axis=0)
    for i in range(len(temp_list)):
        plt.bar(temp_list[i], user_mean_all_gap2[i])
    plt.title("LSTM crowd_mean VS output_mean")
    # 设置 x,y 轴取值范围
    plt.xlabel("samples")
    plt.ylabel("△")
    plt.show()

    user_std_all_gap2 = np.sort(user_std_all_gap, axis=0)
    for i in range(len(temp_list)):
        plt.bar(temp_list[i], user_std_all_gap2[i])
    plt.title("LSTM crowd_std VS output_std")
    # 设置 x,y 轴取值范围
    plt.xlabel("samples")
    plt.ylabel("△")
    plt.show()
...
def main():
    ...
    #filename = 'LSTM_news_user_weight_beta_8_1_1_230714.py'  # 当前路径
    ...
    train_path = open('../dataset/mini_val_data_with_bert_machine_distribution.pkl', 'rb')
    train_file = pickle.load(train_path)

    test_path = open('../dataset/mini_test_data_with_bert_machine_distribution.pkl', 'rb')
    ...
    result_save_path = r'../bert_result_output_is_uniform/'
    encoder_result_save_path = r'../bert_result_output_is_uniform/bert_news_user_weight_beta/gausi_beta_pre_loss/231111/round1_231111/'
    ...
    evaluations=test(user_model=user_model, encoder_model=encoder_model, test_data=test_loader,
         batch_size=test_batch_size_value, forward_passes=forward_passes, vocab=vocab,
         result_save_path=encoder_result_save_path)
    print('machine_acc:', evaluations[0], 'user_acc:', evaluations[1], 'acc:', evaluations[2], 'pre:', evaluations[3], 'rec:',
           evaluations[4], 'f:',evaluations[5], 'auc_result:',evaluations[6])
