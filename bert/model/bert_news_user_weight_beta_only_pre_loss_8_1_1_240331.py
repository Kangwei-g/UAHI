...
def train(encoder_model,  user_model, train_data, ver_data,forward_passes, epoch, batch_size,
    ...
    # 定义损失函数和优化器
    criterion = ELL()
    encoder_optimizer = torch.optim.Adagrad(encoder_model.parameters(), lr=5e-2)
...
def main():
    ...
    #result_save_path = r'../results/only_pre_loss/'
    encoder_result_save_path = r'../results/bert_news_user_weight_beta/only_pre_loss/round2_240331/'
    ...
    train(encoder_model=encoder_model, user_model=user_model, train_data=train_loader,
           ver_data=test_loader, forward_passes=forward_passes, epoch=50, batch_size=train_batch_size_value,
