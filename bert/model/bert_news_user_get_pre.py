...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):  # 重写getitem
        ...
        return text, news_id, label, user,machine_mean,machine_std
...
# 不打开dropout测试
def test(user_model, test_data, batch_size, forward_passes):
    print('test model')
    #encoder_model = encoder_model.cuda()
    user_model = user_model.cuda()
    #encoder_model.eval()
    ...
    user_avg_f = 0
    user_avg_auc = 0
    for idx, (text,news_id, label, user,machine_mean,machine_variance) in enumerate(tqdm(test_data)):
        #train_x = text_transform(text, vocab).cuda()
        ...
        user_mean = user_distribution[:, 0]
        user_variance = user_distribution[:, 1]
        #soft_voting_result= (machine_mean+user_mean)/2

        #encoder_mu, encoder_logstd = encoder_model(machine_mean, machine_variance, user_mean,
        #                                           user_variance)  # pred是mlp输出的二维向量

        #encoder_mu_bool_tensor = torch.gt(soft_voting_result,
        #                                  0.5)  # 使用torch.gt()函数对Tensor的每个元素进行逐元素比较，生成一个布尔型的Tensor，其中值为True的元素表示对应的原Tensor元素大于0.5。
        #encoder_mu_label = torch.where(encoder_mu_bool_tensor, torch.ones_like(soft_voting_result),
        #                               torch.zeros_like(soft_voting_result))  # 逐元素操作，将 True 替换为 1，False 替换为 0
        ...
        #print("test_encoder_mu", soft_voting_result)
        ...
        text.columns = ["text"]
        machine_label_test = pd.DataFrame(int(machine_label_test))
        machine_label_test.columns = ["machine_label_test"]
        user_label_test = pd.DataFrame(int(user_label_test))
        user_label_test.columns = ["user_label_test"]
        y_true = pd.DataFrame(int(y_true))
        y_true.columns = ["y_true"]
        result_df = pd.concat([news_id, text,machine_label_test, user_label_test, y_true])
        #result_duibi = torch.stack([news_id, machine_label_test, user_label_test, y_true], dim=1)
        ...
        #result_duibi.columns = ["news_id", "machine_label_test", "user_label_test","y_true"]
        #result_duibi.to_excel(file_result)
        result_df.to_csv('../results/新闻_用户_预测值和标签/新闻_用户_预测值和文本.csv', index=0)  # 有时间差的测试集
        print("存入成功")
        #file_result.save()
        #file_result.close()
    ...
    '''
        ...
        avg_auc += auc(soft_voting_result, y_true)
...
def main():
    ...
    #filename = 'soft_voting_bert_news_user_weight_beta_8_1_1_240116.py'  # 当前路径
    ...
    encoder_result_save_path = r'../results/bert_news_user_weight_beta/gausi_beta_pre_loss/round1_230716/'
   ...
   # encoder_model.load_state_dict(
    ...
    test(user_model=user_model,  test_data=test_loader,
         batch_size=test_batch_size_value, forward_passes=forward_passes)
