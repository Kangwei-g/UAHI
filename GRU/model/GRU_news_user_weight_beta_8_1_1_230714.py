    ...
    for epoch in tqdm(range(epoch)):
        ...
        if ver_inte_acc > best_inte_acc:
            ...
            torch.save(encoder_model.state_dict(),
                       result_save_path + 'gru_dropout_score_weigt_encoder_model_parameter_bata.pkl')
...
def test(user_model, encoder_model, test_data, batch_size, forward_passes, vocab, result_save_path):
    ...
    for idx, (text, label, user,machine_mean,machine_variance) in enumerate(tqdm(test_data)):
        ...
        machine_avg_pre += precision(machine_label_test, y_true)
        machine_avg_rec += recall(machine_label_test, y_true)
        machine_avg_f += f1(machine_label_test, y_true)
        machine_avg_auc += auc(machine_mean, y_true)
...
def main():
    ...
    filename = 'GRU_news_user_weight_beta_8_1_1_230714.py'  # 当前路径
    ...
    # train_path = open('../dataset/mini_val_data_with_gru_machine_distribution.pkl', 'rb')
    # train_file = pickle.load(train_path)

    test_path = open('../dataset/mini_test_data_with_gru_machine_distribution.pkl', 'rb')
    ...
    encoder_result_save_path = r'../results/gru_news_user_weight_beta/gausi_beta_pre_loss/round1_230714/'
    ...
    encoder_model.load_state_dict(
        torch.load(encoder_result_save_path + 'gru_dropout_score_weigt_encoder_model_parameter_bata.pkl',
                   map_location=torch.device('cpu')))
    print("开始测试————————————————————————————————")
    # 测试结果
