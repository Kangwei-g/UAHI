...
def train(model, train_data, val_data, epoch, result_save_path,model_save_path, vocab, val_size, forward_passes):
    ...
    for epoch in tqdm(range(epoch)):
        ...
        if val_acc > best_acc or (val_acc == best_acc and val_loss < best_loss):
            ...
            # 保存训练完成后的模型参数
            torch.save(model.state_dict(), result_save_path + 'LSTM_news_user_parameter_seed_datasetscore.pkl')
...
def test(model, test_data, vocab, test_size, forward_passes):
    ...
    # file = open(r'../dataset/mini_val_lstm_glove_mean230713.pkl', 'wb')
    # pickle.dump(mean, file)
    # file.close()
    #
    # file2 = open(r'../dataset/mini_val_lstm_glove_std230713.pkl', 'wb')
...
def main():
    ...
    filename = 'LSTM_glove_with_distribution240116.py'  # 当前路径
    ...
    result_save_path = '../results/lstm_glove/round2_230714/'
    model_save_path = '../results/lstm_glove/round2_230714/bin'
    ...
    model.load_state_dict(
        torch.load(result_save_path + 'LSTM_news_user_parameter_seed_datasetscore.pkl', map_location=torch.device('cpu')))
